...
public class MindBodyClientConfig {

	@Value("${mindbody.service.client.url}")
	private String MINDBODY_CLIENT_WEB_SERVICE_URI;

	@Bean
	public Jaxb2Marshaller marshaller() {
		Jaxb2Marshaller marshaller = new Jaxb2Marshaller();
		marshaller.setContextPath("com.fitnessplayground.model.mindbody.api.client");
		return marshaller;
	}

	@Bean
	public MindBodyServiceGateway mindBodyClient(Jaxb2Marshaller marshaller) {
		MindBodyServiceGateway client = new MindBodyServiceGateway();
		client.setDefaultUri(MINDBODY_CLIENT_WEB_SERVICE_URI);
