...
def generate_random_email():
    ...
    return ''.join(random.choices(string.ascii_lowercase, k=8)) + '@' + ''.join(random.choices(string.ascii_lowercase, k=8)) + '.com'


def generate_random_integer(length=5):
    low = 1*10**(length - 1)
    high = 1*10**(length) - 1
    # print(f"{low} - {high}")
    return math.floor(random.uniform(low,high))
